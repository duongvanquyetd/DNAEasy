...
public class NotificationService implements IsNotificationService {
    ...
    public void createNotification(Person p, String content) {
        ...
        p.getNotification().add(notification);
         ...
         Person saved = isUserResponsity.save(p);

        System.out.println("personid : "+p.getPersonId());
         messagingTemplate.convertAndSend("/topic/notification/"+p.getPersonId(),notificationMapper.notitonotresponse(saved.getNotification().getFirst()));
